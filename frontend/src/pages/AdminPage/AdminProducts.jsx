      ...
      if (meta.isUpdate) {
        ...
        if (newCount !== currentCount) {
          if (newCount > currentCount) {
            ...
            notifyDataChange('products', 'มีสินค้าถูกลบออก', 'warning');
          }
        }
  ...
  const deleteProduct = async (productId, productName) => {
    ...
    try {
      ...
      if (error.response?.data?.message) {
        toast.error(`เกิดข้อผิดพลาด: ${error.response.data.message}`);
      } else {
        toast.error("เกิดข้อผิดพลาดในการลบสินค้า");
      }
    }
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Add new product to current list immediately
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setProducts(prevProducts => 
          prevProducts.map(p => 
            p._id === productToEdit._id ? { ...p, ...productToEdit } : p
          )
        );
        ...
        toast.error("อัปเดตสินค้าไม่สำเร็จ");
      }
    } catch (error) {
