  ...
  useEffect(() => {
    const unsubscribe = subscribeToProducts((data, meta) => {
      if (meta.isUpdate) {
        ...
        const newPromotions = productsData.filter(p => p.discount > 0).length;
        
        if (newPromotions !== currentPromotions) {
          if (newPromotions > currentPromotions) {
            notifyDataChange('promotions', 'มีโปรโมชันใหม่!', 'success');
          } else if (newPromotions < currentPromotions) {
            notifyDataChange('promotions', 'โปรโมชันถูกลบ', 'warning');
          } else {
            notifyDataChange('promotions', 'โปรโมชันมีการเปลี่ยนแปลง', 'info');
          }
        }
