      ...
      {selectedImage !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4">
          <div className="relative max-w-4xl w-full">
            <button onClick={closeModal} className="absolute -top-10 right-0 text-white text-2xl">&times;</button>
            <img src={currentImages[selectedImage].src} alt={currentImages[selectedImage].alt} className="w-full h-auto" />
            <div className="absolute inset-x-0 top-1/2 flex justify-between px-4">
              <button onClick={() => navigateImage(-1)} className="bg-white rounded-full p-2 text-gray-800">&larr;</button>
              <button onClick={() => navigateImage(1)} className="bg-white rounded-full p-2 text-gray-800">&rarr;</button>
            </div>
          </div>
        </div>
      )}
...
export default function ProductsDetails({ userId, userData }) {
  ...
  const handleAddToBag = async () => {
    // ตรวจสอบว่าผู้ใช้เลือกสีและขนาดแล้ว
    if (!selectedColor || !selectedSize) {
        alert("กรุณาเลือกสีและขนาดสินค้า");
        return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to add product to bag:", result.message);
        
        // ถ้าต้องการการยืนยันตัวตน ให้เด้งไปหน้าสมัครสมาชิก
